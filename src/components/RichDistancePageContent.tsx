...
/**
 * Visually distinct "rich" distance-page template — used ONLY for the 5
 * Tuscany/Lazio routes this variant was built for. Deliberately does not
 * reuse DistancePageContent's navy-hero/stat-card grammar: a warm stone
 * background throughout, an integrated hero+map opening (rather than a
 * banner followed by disconnected content), a horizontal stat strip instead
 * of boxed cards, a connected-dot journey timeline, and a transport
 * comparison table instead of stacked paragraphs.
 */
export default function RichDistancePageContent({ page }: { page: RichDistancePage }) {
    const { origin, dest, h1, slug } = page;
    const accent = JOURNEY_TYPE_COLOR[page.journeyType] ?? '#7A6A4F';

    const webPageSchema = {
        '@context': 'https://schema.org', '@type': 'WebPage',
        name: h1, url: `${SITE}/distance/${slug}`,
        ...
        <main className="font-inter bg-[#FAF7F1] text-[#1E2A22]">
            ...
            <section className="pt-32 pb-10 px-6 border-b border-[#E7DFCE]">
                <div className="container mx-auto max-w-5xl">
                    <nav aria-label="Breadcrumb" className="flex items-center gap-2 text-xs text-[#8A8067] mb-8">
                        <Link href="/" className="hover:text-[#1E2A22]">Home</Link>
                        <span>/</span>
                    ...
                    <div className="flex flex-wrap gap-8">
                        <div>
                            <div className="text-[11px] font-bold uppercase tracking-widest text-[#8A8067] mb-1">Road Distance</div>
                            <div className="text-2xl font-extrabold text-[#1E2A22]">{page.drivingDistanceKm}</div>
                        </div>
                        <div className="w-px bg-[#E7DFCE] self-stretch hidden sm:block" />
                        <div>
                            <div className="text-[11px] font-bold uppercase tracking-widest text-[#8A8067] mb-1">Driving Time</div>
                            <div className="text-2xl font-extrabold text-[#1E2A22]">{page.drivingDurationRange}</div>
                        </div>
                    </div>
                </div>
            </section>
            ...
            <section className="py-10 px-6 border-b border-[#E7DFCE]">
                <div className="container mx-auto max-w-5xl">
                    <h2 className="text-xl font-bold text-[#1E2A22] mb-1 flex items-center gap-2">
                        <MapPin className="w-5 h-5" style={{ color: accent }} /> The Route
                    </h2>
                    <p className="text-sm text-[#8A8067] mb-5">Real road-network route between {origin} and {dest} — not a straight line between two points.</p>
                    <RouteMapLoader origin={page.map.origin} destination={page.map.destination} geometry={page.map.geometry} />
                    <p className="text-xs text-[#8A8067] mt-3 flex items-start gap-1.5">
                        <Info className="w-3.5 h-3.5 shrink-0 mt-0.5" />
                        The route shown reflects a typical road path — your actual journey can vary depending on traffic, road conditions, and your exact pickup or drop-off location.
                    </p>
                </div>
            </section>
            ...
            <div className="container mx-auto max-w-3xl px-6">
                ...
                <section className="py-12 border-b border-[#E7DFCE]">
                    <h2 className="text-2xl font-bold text-[#1E2A22] mb-5">How Far Is {dest} from {origin}?</h2>
                ...
                {/* ── Journey timeline ──────────────────────────────────────── */}
                <section className="py-12 border-b border-[#E7DFCE]">
                    <h2 className="text-2xl font-bold text-[#1E2A22] mb-2">How Long Does the Drive Take?</h2>
                    <p className="text-[#8A8067] text-sm mb-8">{page.drivingDurationRange} under normal conditions.</p>
                ...
                <section className="py-12 border-b border-[#E7DFCE]">
                    <h2 className="text-2xl font-bold text-[#1E2A22] mb-5">Understanding the Journey</h2>
                ...
                {/* ── Journey highlights ────────────────────────────────────── */}
                <section className="py-12 border-b border-[#E7DFCE]">
                    <h2 className="text-2xl font-bold text-[#1E2A22] mb-6">Journey Highlights</h2>
                ...
                {/* ── Best ways to travel ───────────────────────────────────── */}
                <section className="py-12 border-b border-[#E7DFCE]">
                    <h2 className="text-2xl font-bold text-[#1E2A22] mb-6">Best Ways to Travel</h2>
                    <div className="overflow-x-auto -mx-6 px-6 sm:mx-0 sm:px-0">
                        <table className="w-full border-collapse text-sm min-w-[480px]">
                            <thead>
                                <tr className="border-b-2 border-[#1E2A22]">
                                    <th className="text-left py-2.5 pr-3 font-bold text-[#1E2A22]">Option</th>
                                    <th className="text-left py-2.5 pr-3 font-bold text-[#1E2A22]">Time</th>
                                    <th className="text-left py-2.5 font-bold text-[#1E2A22]">Notes</th>
                                </tr>
                            </thead>
                            <tbody>
                ...
                <section className="py-12 border-b border-[#E7DFCE]">
                    <h2 className="text-2xl font-bold text-[#1E2A22] mb-6">Planning Your Journey</h2>
            ...
            <FAQSection faqs={page.faqs} title={`${origin} to ${dest} — Frequently Asked Questions`} />

            {/* ── Related journeys ──────────────────────────────────────────── */}
            <section className="py-14 px-6 bg-[#F3EEE1]/60">
                <div className="container mx-auto max-w-5xl">
                    <h2 className="text-xl font-bold text-[#1E2A22] mb-6">Related Journeys</h2>
